...
public class P2Wrapper {
	...
	public static class SimpleBinaryTreeImp<E> implements SimpleBinaryTree<E> {
		...
		//
		/*
		 * A Binary Tree is symmetric if and only if for every internal node:
		 * a) the value at the left child is equal to the value at the right child
		 * b) the subtree rooted at the left child is symmetric 
		 * b) the subtree rooted at the right child is symmetric.
		 * 
		...
		public boolean isSymmetric() {
			...
			if(this.root.leftChild != null && this.root.rightChild != null) {
				if(this.root.leftChild.value == this.root.rightChild.value) {
					return true;
				}
				return false;
			}
